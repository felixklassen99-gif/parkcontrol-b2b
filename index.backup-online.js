...
const tr = {
  ...
  pt: {
    ...
    back: 'Voltar',
    denied: 'Sem acesso',
    employeeHint: 'Employee só pode ver entrada, saída, tickets e contrato.'
  }
};
...
app.get('/subscription', requireAuth, async (req, res) => {
  ...
  if (req.session.user.role === 'superadmin') {
    ...
    return res.send(page(t(req, 'contract'), `
      <div class="card">
        ...
        <table>
          <tr>
            <th>ID</th><th>Firma</th><th>Plan</th><th>Status</th><th>Monatsgebühr</th><th>Fälligkeit</th><th>Letzte Zahlung</th><th>Gesperrt</th>
          </tr>
          ${companies.map(x => `
            <tr>
              <td>${x.id}</td>
  ...
  res.send(page(t(req, 'contract'), `
    <div class="card small">
      ...
      <p><strong>Monatsgebühr:</strong> ${money(c?.monthly_fee || 0)}</p>
      <p><strong>Fälligkeit:</strong> ${esc(c?.next_due_date || '')}</p>
  ...
  res.send(page(t(req, 'companies'), `
    <div class="grid">
      ...
      <div class="card">
        ...
        <table>
          <tr>
            <th>ID</th><th>Firma</th><th>Plan</th><th>Status</th><th>Monatsgebühr</th><th>Fälligkeit</th><th>Gesperrt</th><th>Aktion</th>
          </tr>
          ${companies.map(c => `
            <tr>
              <td>${c.id}</td>
  ...
  res.send(page(t(req, 'entry'), `
    <div class="card small">
      ...
      <form method="post" action="/entry">
        ...
        <label>Monatsmieter wählen</label>
...
app.post('/exit', requireWork, async (req, res) => {
  ...
  res.send(page('Ausfahrt abgeschlossen', `
    <div class="card small">
      ...
      <a class="button-link" href="/exit">Nächstes Ticket</a>
    </div>
  `, req.session.user, req.currentCompany, req));
});
